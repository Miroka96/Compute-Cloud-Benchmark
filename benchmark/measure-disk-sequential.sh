...
WRITE=$(dd if=/dev/zero of=seq_write bs=250M count=8 conv=fdatasync 2>&1 | tail -n 1 | 
cut -d " " -f 10-11 | awk '{if($2 ~ /GB/){print $1 * 1000000000}else{print $1 * 1000000}}')
# Testing read
...
READ=$(dd if=seq_write of=/dev/null bs=250M count=8 conv=fdatasync | 2>&1 | tail -n 1 | 
cut -d " " -f 10-11 | awk '{if($2 ~ /GB/){print $1 * 1000000000}else{print $1 * 1000000}}')
